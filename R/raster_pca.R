...
raster_pca <- function(raststack, aggregate = 0, focal = 0) {
    # Computing PCA
    rasterPCA <- princomp(na.omit(raster::values(raststack)),
                          cor = T, scores = T)
    ...
    summaryPCA <- as.data.frame(
        rbind(SD <- sqrt(rasterPCA$sdev^2),
        proportion <- rasterPCA$sdev^2 / sum(rasterPCA$sdev^2),
        cumulative <- cumsum(rasterPCA$sdev^2) / sum(rasterPCA$sdev^2)))
    ...
    if (aggregate != 0) {
        ...
        if (focal != 0) {
            aggfiltPCA <- multiFocal(aggregatePCA, w = matrix(1, focal, focal), fun = "mean", na.rm = T)
        ...
        if (focal != 0) {
            filtPCA <- multiFocal(stackrasterPCA, w = matrix(1, focal, focal), fun = "mean", na.rm = T)
